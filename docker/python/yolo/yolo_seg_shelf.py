...
# shelf
affine_matrix = np.array([[ 1.21864674e+00, -1.39789051e-02, -1.83644663e+01],
                        [ 1.10728237e-02,  1.21577368e+00, -6.46403774e+01]])


path = '/images/data/0531/shelf/*jpg'

file_list = peekable(sorted(glob.iglob(path)))

fourcc = cv2.VideoWriter_fourcc('m', 'p', '4', 'v')
video = cv2.VideoWriter('test_shelf_0531.mp4',fourcc, 30.3, (640, 480), isColor=True)
...
# gray_bg = cv2.cvtColor(cv2.imread('/images/data/0527/bg_table_V.jpg'), cv2.COLOR_BGR2GRAY)
...
for i in file_list:
    try:
        if '_V' in i and '_T' in file_list.peek():
            ...
            _, img_th_t = cv2.threshold(affined_t,5,255,cv2.THRESH_BINARY)
            ...
            # with open("./log/test_shelf_0527.csv", "a") as f:
            ...
            #                 path = '../results/shelf_0527_thumbnails/{}'.format(int(cls))
            #                 if not os.path.exists(path): os.makedirs(path)
            #                 path = '../results/shelf_0527_detail/{}'.format(int(cls))
            #                 if not os.path.exists(path): os.makedirs(path)
            #                 path = '../results/shelf_0527_mask/{}'.format(int(cls))
            #                 if not os.path.exists(path): os.makedirs(path)
                            
            #                 xmin, ymin, xmax, ymax = map(int, bbox[:4])
            #                 crop = img_v_color[ymin:ymax, xmin:xmax]
            #                 cv2.imwrite('../results/shelf_0527_thumbnails/{}/{}.jpg'.format(int(cls),time), crop)
                            
            #                 overview = img_v_color.copy()
            #                 cv2.rectangle(overview, (xmin,ymin), (xmax,ymax), (0, 0, 255), thickness=5)
            #                 cv2.imwrite('../results/shelf_0527_detail/{}/detail_{}.jpg'.format(int(cls),time), overview)
                            
            #                 mask = cv2.bitwise_and(erode_v, erode_t)
            #                 mask_inv = cv2.bitwise_not(mask)[ymin:ymax, xmin:xmax]
            #                 cv2.imwrite('../results/shelf_0527_mask/{}/{}.jpg'.format(int(cls),time), mask_inv)
            #                 break
