...
video = cv2.VideoWriter('test_shelf4.mp4',fourcc, 30.3, (640, 480), isColor=True)
...
for i in file_list:
    try:
        if '_V' in i and '_T' in file_list.peek():
            ...
            with open("./log/test_table.csv", "a") as f:
                for poly, cls, bbox in zip(polygon, classes, bboxes):
                    for pt in points:
                        if cv2.pointPolygonTest(poly, pt, False) >= 0 and cls != 0:
                            ...
                            path = '../results/table_thumbnails/{}'.format(int(cls))
                            if not os.path.exists(path): os.makedirs(path)
                            path = '../results/table_detail/{}'.format(int(cls))
                            if not os.path.exists(path): os.makedirs(path)
                            path = '../results/table_mask/{}'.format(int(cls))
                            if not os.path.exists(path): os.makedirs(path)
                            
                            xmin, ymin, xmax, ymax = map(int, bbox[:4])
                            crop = img_v_color[ymin:ymax, xmin:xmax]
                            cv2.imwrite('../results/table_thumbnails/{}/{}.jpg'.format(int(cls),time), crop)
                            
                            overview = img_v_color.copy()
                            cv2.rectangle(overview, (xmin,ymin), (xmax,ymax), (0, 0, 255), thickness=5)
                            cv2.imwrite('../results/table_detail/{}/detail_{}.jpg'.format(int(cls),time), overview)
                            
                            mask = cv2.bitwise_and(erode_v, erode_t)
                            mask_inv = cv2.bitwise_not(mask)[ymin:ymax, xmin:xmax]
                            cv2.imwrite('../results/table_mask/{}/{}.jpg'.format(int(cls),time), mask_inv)
                            break
